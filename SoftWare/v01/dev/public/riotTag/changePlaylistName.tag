<changeplaylistname>
  ...
  <div class="changeName">
    <form class="modal-3HOjGZ sizeSmall-1sh0-r">
      ...
      <div class="scrollerWrap-2uBjct content-1Cut5s scrollerThemed-19vinI themeGhostHairline-2H8SiW">
        <div class="scroller-fzNley inner-tqJwAU">
          <div class="input-3VBhvk marginBottom8-1mABJ4">
            ...
            <div class="inputWrapper-3xoRWR vertical-3X17r5 flex-3B1Tl4 directionColumn-2h-LPR">
              <input id='nName' type="text" class="inputDefault-Y_U37D input-2YozMi size16-3IvaX_" value="" placeholder={playlistName} name="" maxlength="999">
              </div>
            </div>
        ...
        <div class="flex-lFgbSz flex-3B1Tl4 horizontalReverse-2LanvO horizontalReverse-k5PqxT flex-3B1Tl4 directionRowReverse-2eZTxP justifyStart-2yIZo0 alignStretch-1hwxMa noWrap-v6g9vO footer-1PYmcw" style="flex: 0 0 auto;">
          <button type="button" onclick={changeName} class="button-2t3of8 lookFilled-luDKDo colorBrand-3PmwCE sizeMedium-2VGNaF grow-25YQ8u">
            <div class="contents-4L4hQM">Enregistrer</div>
          </button>
          <button onclick={close} type="button" class="button-2t3of8 lookLink-3VWONr colorPrimary-2KuK5O sizeMedium-2VGNaF grow-25YQ8u">
    ...
    var rqMng = require(__dirname + '/../js/utils/requestManager.js');

    console.log('rqMng : ', rqMng);
    ...
    console.log('sombre fils de pute: ', rqMng.request);
    ...
    changeName(e) {
      ...
      var data = JSON.stringify({playlist_id : this.id
        , playlist_name : $('#nName').val()});

      requestManager.request(baseURL, options, data, function (rslt, req, err) {
        console.log('requete spece de iench : ', rslt.rslt);
          if (rslt.rslt.code == 0) {
            console.log('name correctly changed: ', rslt);
            //self.unmount(true);
            self.playlistName = rslt.rslt.Playlist.playlist_name;
            console.log('nName: ', self.playlistName);

            myEmitter.emit('plNameChange'
            , {id : self.id, name : self.playlistName});
            self.unmount(true);
          }
        })
      };
