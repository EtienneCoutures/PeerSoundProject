  ...
  module.exports = function(app) {
    ...
    router.get('/',
      ...
      function(req, res) {
        ...
        // filters
        if (!S(Options.where.invited_usr_id).isEmpty()) query.where.invited_usr_id = {
          like: Options.where.invited_usr_id
        };
        if (!S(Options.where.inviter_usr_id).isEmpty()) query.where.inviter_usr_id = {
          like: Options.where.inviter_usr_id
        };
        if (!S(Options.where.playlist_id).isEmpty()) query.where.playlist_id = {
          like: Options.where.playlist_id
        };
        if (!S(Options.where.invited_role).isEmpty()) query.where.invited_role = {
          like: Options.where.invited_role
        };
      ...
      function(req, res) {
        ...
        function createRecord() {
          ...
          app.models["Playlist"].findOne({
            ...
            include: [{
              ...
              required: false,
              where: {
                usr_id: Record.invited_usr_id
              }
            }]
          }).then(function(ret) {
            if (ret && ret.dataValues.Subscriber[0]) {
              return reply(null, {
                code: 2,
                message: "user already in pl"
              })
            }
        ...
        function reply(err, record) {
          if (err) {
            res.json({
              code: -1,
              errors: (err.errors && err.errors.length > 0) ? err.errors : [{
    ...
    router.delete('/:id',
      ...
      function(req, res) {
        app.models["Invitation"].find({
          ...
          res.json({
            code: 0,
            message: req.translate('system', '__MODEL__ successfully deleted', {
